...
Деструктор выполняет **линейный обход** с удалением каждого узла. Критичен порядок операций: `next` сохраняется до `delete`, иначе — обращение к освобождtнной памяти (undefined behaviour).
